<template>
  <a-breadcrumb style="">
    <a-breadcrumb-item>
      <RouterLink :to="{ path: '/' }"> Trang chủ </RouterLink>
    </a-breadcrumb-item>
...
const getParamsRoute = () => {
  ...
  switch (
    route.path
      .replaceAll(route.params?.slug ? (route.params?.slug as string) : '', '')
      .replaceAll(
        route.params?.slug2 ? (route.params?.slug2 as string) : '',
        ''
      )
      .replaceAll(route.params?.id ? (route.params?.id as string) : '', '')
      .replaceAll(route.params?.name ? (route.params?.name as string) : '', '')
      .replaceAll(route.params?.tap ? (route.params?.tap as string) : '', '')
      .replaceAll('/', '')
  ) {
    case 'discover':
